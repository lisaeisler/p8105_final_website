...
As expected, the majority of patients had neuromuscular-type scoliosis, which results from medical conditions that impair a child's ability to control the muscles that support the spine such as cerebral palsy.
```{r, echo=FALSE, message=FALSE, warning=FALSE}
cp_spine_tidy %>%
  count(ped_spn_class) %>% 
  replace(is.na(.), 0) %>% 
...
## Associations Between Pre- and Intra- Operative Factors and Outcomes
We wanted to explore whether any individual potential risk factors affected different outcomes. The following plots illustrate some of the relationships we explored.
...
Because there are so many different variables and potential relationships to explore, we decided to focus on an outcome variable that was clinically of interest to all three team members working on the project. We chose the variable home_discharge, which distinguishes whether or not each patient was able to be discharged to home after surgery, rather than to a care facility such as a rehabilitation center. 
